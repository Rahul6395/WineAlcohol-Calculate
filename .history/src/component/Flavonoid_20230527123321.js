...
function mean(data,classN){
  ...
  let reduce = dataFilter.reduce((a,b)=>a+b.Flavanoids,0)
  console.log("reduce",reduce)
  return parseInt(reduce)/dataFilter.length
}


 classNames.map((classN)=>{
 console.log("mean(data,classN)",mean(data,classN)) 
} )
  ...
  function calculateMean(data, className) {
    ...
    const sum = filteredData.reduce((acc, item) => acc + item.Flavanoids, 0)/ filteredData.length;
    return parseInt(sum) ;
  }
