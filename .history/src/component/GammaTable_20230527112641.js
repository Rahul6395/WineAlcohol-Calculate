...
const GammaTable = () => {
  ...
  const calculateGammaStats = () => {
    ...
    const statsByClass = Object.entries(gammaByClass).map(([className, gammaValues]) => {
      // Mean
        // console.log("calculateMeangammaValues",gammaValues)
      const mean = gammaValues.reduce((sum, value) => sum + value, 0) / gammaValues.length;
      // Median
      let sortData = gammaValues.sort((a, b) => a - b);
      const midIndex = Math.floor(sortData.length / 2);
      const median = sortData.length % 2 === 0 ? (sortData[midIndex-1] +  sortData[midIndex]) / 2 : sortData[midIndex] 
    ...
    <table>
      ...
      <tbody>
        ...
        <tr>
          <td>Gamma Median</td>
          {gammaStats.map((stat) => (
            <td key={stat.className}>{console.log("stat.median",stat.median)}{stat.median}</td>
          ))}
        </tr>
        <tr>
          <td>Gamma Mode</td>
