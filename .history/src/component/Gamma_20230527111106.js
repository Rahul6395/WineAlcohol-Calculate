    ...
    //   Median
      function calculateMedian( className,data,) {
        ...
        if (sortedData.length % 2 === 0) {
          return console.log("if",(sortedData[midIndex - 1]?.Flavanoids + sortedData[midIndex]?.Flavanoids) / 2);
        } else {
          return console.log("else",sortedData[midIndex].Flavanoids);
        }
      }
    ...
    <table>
      ...
      <tbody>
        ...
        <tr>
          ...
          {
          Object.entries(gammaByClass).map(([className, gammaValues])=>(
            <td>{calculateMedian( className,gammaValues)}</td>
          ))
        }
        </tr>
        <tr>
